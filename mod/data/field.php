    ...
    if (!$DB->record_exists('data_fields', array('dataid'=>$data->id))) {
        ...
        if ($fff = $DB->get_records('data_fields', array('dataid'=>$data->id),'id')){
            foreach ($fff as $ff) {
                ...
                '<img src="'.$OUTPUT->pix_url('t/edit') . '" class="icon-small" alt="'.get_string('edit').'" title="'.get_string('edit').'" /></a>'.
                '&nbsp;'.
                '<a href="field.php?d='.$data->id.'&amp;mode=delete&amp;fid='.$field->field->id.'&amp;sesskey='.sesskey().'">'.
                '<img src="'.$OUTPUT->pix_url('t/delete') . '" class="icon-small" alt="'.get_string('delete').'" title="'.get_string('delete').'" /></a>'
