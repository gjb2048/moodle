...
class mod_lesson_renderer extends plugin_renderer_base {
    ...
    public function page_action_links(lesson_page $page, $printmove, $printaddpage=false) {
        ...
        if ($printmove) {
            ...
            $actions[] = html_writer::link($printmovehtml, '<img src="'.$this->output->pix_url('t/move').'" class="icon-small" alt="'.get_string('move').'" />');
        }
        $url = new moodle_url('/mod/lesson/editpage.php', array('id'=>$this->page->cm->id, 'pageid'=>$page->id, 'edit'=>1));
        $actions[] = html_writer::link($url, '<img src="'.$this->output->pix_url('t/edit').'" class="icon-small" alt="'.get_string('update').'" />');

        $url = new moodle_url('/mod/lesson/view.php', array('id'=>$this->page->cm->id, 'pageid'=>$page->id));
        $actions[] = html_writer::link($url, '<img src="'.$this->output->pix_url('t/preview').'" class="icon-small" alt="'.get_string('preview').'" />');

        $url = new moodle_url('/mod/lesson/lesson.php', array('id'=>$this->page->cm->id, 'action'=>'confirmdelete', 'pageid'=>$page->id, 'sesskey'=>sesskey()));
        $actions[] = html_writer::link($url, '<img src="'.$this->output->pix_url('t/delete').'" class="icon-small" alt="'.get_string('delete').'" />');
